...
DEFAULT_SONG_GROUP  equ 9999999 ; 默认组别被分配到的编号 ; 
DEFAULT_PLAY_SONG   equ 8888888 ; 默认的第index首歌 ;
...
MAX_SONG_NAME_LEN equ 150; 最大歌曲的名字的长度 
...
findNone byte "您没有选中歌单，不能进行本地搜索", 0
...
;	mov	hiword, 
	shrd eax, ebx, 16
	mov	hiword, ax
	mov	eax, lParam ; WParam = (hiword, lowrd) : 详见notification code
	mov	lloword, ax
;	mov	hiword, 
	shrd eax, ebx, 16
	mov	lhiword, ax
...
ChangeTheme proc, 
	...
	; 显示背景并把当前设置为不可见
	.if curTheme == 0 ; 隐藏当前背景
		invoke GetDlgItem, hWin, IDC_BACKGROUND
		invoke ShowWindow, eax, SW_SHOW ; 显示目标背景
		invoke GetDlgItem, hWin, IDC_BACKGROUND_ORANGE
		invoke ShowWindow, eax, SW_HIDE ; 对应图标放置到控件
		...
		invoke SendDlgItemMessage, hWin, IDC_CLOSE, BM_SETIMAGE, IMAGE_ICON, ico_Cross_Blue ; 固定背景bitmap
...
FIND_MILLISECOND:
	...
	push eax ; 将分钟转化为millisecond
	mov eax, timeMinute
	mul const60
	mul const1000
	add timeMillisecond, eax
	mov eax,timeSecond ; 将秒钟转化为millisecond
	mul const1000
	add timeMillisecond, eax
	pop eax
	...
	pop (lyric ptr [esi]).timeStamp ; 记录时间戳
	mov esi, lyricSavePos
	add esi, size dword
	mov strPos, esi
	pop esi
